...
class UserModel extends BaseModel {
  ...
  static async checkLogin({ email, password }) {
    const user = await this.query().where('Active_status',1).findOne({ email: email });
  ...
  static async veriy2FA({ code, tokenVerify }: { code: string, tokenVerify: string }) {
    const user = await this.query().where('Active_status',1).findOne({ code: code });
    if (!user) return false;
  ...
  static async getInfoAuth(auth) {
    let result = await this.query().withGraphJoined('group').where('Active_status',1).where('users.id', auth.id).first()
