...
import userTempService from '@root/src/services/userTempService';
...
const Edit = () => {
  ...
  const fetchData = async() => {
    ...
    let [adminError, admin]: [any, any] = await to(userService().withAuth().detail({ id: query.id }));
    admin.twofa = admin.twofa ? true : false
    admin.birthday = admin?.birthday ? moment(admin.birthday) : ''
  ...
  const onFinish = async (values: any): Promise<void> => {
    setLoading(true)
    if(updatePer){
      let [error, result]: any[] = await to(userTempService().withAuth().edit({
        id: admin.id,
        ...values
      }));
      setLoading(false)
